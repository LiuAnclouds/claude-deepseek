...
if (-not $env:ANTHROPIC_BASE_URL) {
  $env:ANTHROPIC_BASE_URL = 'https://llmapi.horizon.auto'
}
if (-not $env:ANTHROPIC_MODEL) {
  $env:ANTHROPIC_MODEL = 'HORIZON-DeepSeek-Pro'
}
if (-not $env:ANTHROPIC_DEFAULT_OPUS_MODEL) {
  $env:ANTHROPIC_DEFAULT_OPUS_MODEL = 'HORIZON-DeepSeek-Pro'
}
if (-not $env:ANTHROPIC_DEFAULT_SONNET_MODEL) {
  $env:ANTHROPIC_DEFAULT_SONNET_MODEL = 'HORIZON-GLM'
}
if (-not $env:ANTHROPIC_DEFAULT_HAIKU_MODEL) {
  $env:ANTHROPIC_DEFAULT_HAIKU_MODEL = 'HORIZON-DeepSeek-Pro'
}
if (-not $env:CLAUDE_CODE_SUBAGENT_MODEL) {
  $env:CLAUDE_CODE_SUBAGENT_MODEL = 'HORIZON-GLM'
}
...
if (Test-Path $ModelsFile) {
  $modelLines = Get-Content -LiteralPath $ModelsFile -ErrorAction SilentlyContinue
...
if (-not $env:ANTHROPIC_AUTH_TOKEN) {
  if ($Host.Name -eq 'ConsoleHost') {
    ...
    throw 'No Horizon API key configured. Run claude-horizon-config.'
  }
}
